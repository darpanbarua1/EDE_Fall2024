...
output: 
  pdf_document:
    ...
    fig_caption: true     # This enables Figure Captions
  lof: true 
---
...
1.  Rename this file `<DarpanBarua>_A06_CraftingReports.Rmd` (replacing `<FirstLast>` with your first and last name).
...
#Import EPA data & fix dates
epa_data <- read.csv(
  here("Data", "Processed", "EPAair_O3_PM25_NC1819_Processed.csv"),
  stringsAsFactors = TRUE
) %>%
  mutate(Date = ymd_hms(Date))  # Correctly parse Date-Time format
```
...
summary_PM25 <- epa_data %>%
  drop_na(AQI_PM2.5) %>%
  group_by(COUNTY) %>%
  summarize(mean_PM2.5 = round(mean(AQI_PM2.5), 2)) %>%
  arrange(mean_PM2.5)
#Compute mean ozone concentration for each county, sorted by value
summary_Ozone <- epa_data %>%
  drop_na(AQI_Ozone) %>%
  group_by(COUNTY) %>%
  summarize(mean_Ozone = round(mean(AQI_Ozone), 2)) %>%
  arrange(mean_Ozone)
...
```{r ozone_2018, echo=FALSE, warning=FALSE, fig.cap="Boxplot of Ozone Levels by Month for 2018"}
...
ggplot(ozone_2018, aes(x = factor(format(Date, "%m")), y = AQI_Ozone)) +
  geom_boxplot(fill = "skyblue", color = "black") +
  labs(title = "Ozone Levels (2018)", x = "Month", y = "Ozone Levels") +
...
```{r ozone_2019, echo=FALSE, warning=FALSE, fig.cap=" Boxplot of Ozone Levels by Month for 2019"}
...
ggplot(ozone_2019, aes(x = factor(format(Date, "%m")), y = AQI_Ozone)) +
  geom_boxplot(fill = "lightcoral", color = "black") +
  labs(title = "Ozone Levels (2019)", x = "Month", y = "Ozone Levels") +
  theme_minimal(base_size = 14) + 
  theme(panel.background = element_rect(fill = "grey90"))
